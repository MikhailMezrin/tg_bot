...
def start(message):
    ...
    bot.send_message(message.chat.id, 'Привет, {0.first_name}.Дорогая, сегодня 3 месяца с момента того, как мы познакомились. Мне не хватит и всех слов мира, чтобы описать насоклько сильно я люблю тебя, но тут я подобрал несколько.'.format(message.from_user), reply_markup=markup)
